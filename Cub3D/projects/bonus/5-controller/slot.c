...
void	init_slot(void)
{
	...
	game = get_game();
	init_animation(&game->img->knife[0], 0, 15, 2, KNF_TAKE);
	init_animation(&game->img->knife[1], 0, 10, 2, KNF_IDLE);
	init_animation(&game->img->vandal[0], 0, 15, 2, GUN_TAKE);
	init_animation(&game->img->vandal[1], 0, 20, 2, GUN_IDLE);
	init_animation(&game->img->vandal[2], 0, 80, 2, GUN_SKIN);
	init_animation(&game->img->qskill[0], 0, 07, 2, QSK_TAKE);
	init_animation(&game->img->qskill[1], 0, 35, 2, QSK_IDLE);
	init_animation(&game->img->qskill[3], 0, 05, 2, QSK_FIRE);
	init_animation(&game->img->rskill[3], 2, 10, 2, RSK_FIRE);
