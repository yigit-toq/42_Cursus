...
/*   Updated: 2025/07/25 22:58:28 by ytop             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			Bureaucrat::executeForm(AForm const & form) // new
{
	try
	{
		form.execute(*this);
		std::cout << W_CLR << this->name << G_CLR << " executed " << Y_CLR << form.getName() << RESET << std::endl;
	}
	catch (std::exception &e)
	{
		std::cout << W_CLR << this->name << R_CLR << " couldn't execute " << Y_CLR << form.getName() << " because: " << e.what() << RESET << std::endl;
	}
}
