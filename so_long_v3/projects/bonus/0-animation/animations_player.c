...
/*   Updated: 2025/01/16 18:54:38 by ytop             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	set_variable(t_player *player, char direction, int **transform);
...
void	player_animation(t_game *game)
{
	...
	if (game->player->frame == 3)
		game->player->frame = 0;
	sign = set_variable(game->player, game->player->direction, &transform);
	*transform += PLAYER_SPEED * sign;
	...
	if (progress == (CONST / PLAYER_SPEED))
	{
		...
		game->player->move = 0;
		game->player->frame = 0;
		set_variable(game->player, game->player->direction, &transform);
...
static int	player_movement(t_game *game)
{
	...
	player = game->player;
	count = game->count;
	if (game->map->map[player->n_t[1]][player->n_t[0]] == COLL)
	{
		count->coll--;
		count->score++;
		if (!count->coll)
