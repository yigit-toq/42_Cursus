...
/*   Updated: 2025/01/16 19:16:23 by ytop             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	wall_control(t_map *map, int x, int y, char **path)
{
	...
	{
		array_free(path, map->h);
		array_free(map->map, map->h);
...
{
	if (!ft_strchr("01PEC", map->map[y][x]))
	{
		array_free(path, map->h);
		array_free(map->map, map->h);
		ft_printf("%sHeight: %d, Width: %d\n%s", C_B, y + 1, x + 1, END);
...
void	object_control(t_game *game)
{
	...
	if (!game->count->coll)
		error_controller(game, 'M', "There is no coll.", 0);
	if (!game->count->exit)
		error_controller(game, 'M', "There is no exit.", 0);
	if (game->count->exit > E_COUNT)
