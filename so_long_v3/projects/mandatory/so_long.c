...
/*   Updated: 2025/01/16 18:40:01 by ytop             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	mlx_controller(t_game *game, int width, int height)
{
	...
	game->win = win;
	image_init(game);
	mlx_hook(win, 3, (1L << 1), key_controller, game);
...
static void	image_init(t_game *game)
{
	game->img->floor = new_image(game, GRND_IMG);
	game->img->coll = new_image(game, COLL_IMG);
